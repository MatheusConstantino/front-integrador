...
export const requestHandlerError = error => {
  ...
  if (_.isEqual(error.status, 401)) {
    return data ? (data.message ? data.message : data) : MENSAGEM_UNAUTHORIZED;
  }
  if (_.isEqual(error.status, 404)) {
    return MENSAGEM_NOT_FOUND;
  }
  ...
  return data
    ? (errMsg = data.error ? data.error : data)
    : (errMsg = MENSAGEM_ERRO);
};
